...
-- 4. Désactiver RLS (Row Level Security) pour faciliter l'accès depuis le backend
-- (Pour une production sécurisée, il faudrait configurer des politiques RLS)
ALTER TABLE public.users ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.amendes ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.rapports ENABLE ROW LEVEL SECURITY;

-- Créer des polices permissives pour permettre l'accès public (puisqu'on gère la sécurité dans notre backend)
